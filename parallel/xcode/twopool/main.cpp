...
#define exportFolderName    "/Users/stefanhallermann/Desktop/github/parallel/out/"
//-------------------------------------------------------------------------------------------------------------------------
...
#define howManyExperiments 6 //first three experiments before and after
...
//pr model Tsodyks & Markram PNAS 1997
...
float minisize[100];    //size in miniature EPSC in pA for each experiment; is here set constant to 15.2 pA in load()
...
void load(){
    ...
    for(ii=0;ii<howManyExperiments;ii++) {
        minisize[ii]=15.2; //assumung 15.2 pA mini size
    }
...
void setRates() 
{
    ...
    n1tot = ptry[1];
    U20 = ptry[2];
    if(U20>1) {U20=1.;ptry[2]=1.;};//Pr cannot be >1
    k2 = ptry[3];
    k1 = ptry[4];
    
    //values dereived from fits to data with various frequency as shown in Fig. S5B
    tauF = 0.0293833;
    k0 = 2.64102;
    nSupplPerN2 = 2.94517;
...
int main(int argc, char* argv[])
{
    ...
    //define start values that should be optimized based on fits to data with different frequency as shown in Fig. S5B
    indexTmp = 0;
    startMerk[indexTmp] = 20*0.296059;
    startDeltaMerk[indexTmp] = 0.5 * startMerk[indexTmp];

    indexTmp += 1;
    startMerk[indexTmp] = 20*(1-0.296059);
    startDeltaMerk[indexTmp] = 0.5 * startMerk[indexTmp];

    indexTmp += 1;
    startMerk[indexTmp] = 0.677206;
    startDeltaMerk[indexTmp] = 0.1 * startMerk[indexTmp];

    indexTmp += 1;
    startMerk[indexTmp] = 1.53747;
    startDeltaMerk[indexTmp] = 0.1 * startMerk[indexTmp];

    indexTmp += 1;
    startMerk[indexTmp] = 45.5643;
    startDeltaMerk[indexTmp] = 0.1 * startMerk[indexTmp];
