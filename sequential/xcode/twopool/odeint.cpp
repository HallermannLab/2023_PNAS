	...
	Vec_I_DP &, DP &, DP &, void (*)(const DP, Vec_I_DP &, Vec_O_DP &)))
{
	...
	for (nstp=0;nstp<MAXSTP;nstp++) {
		...
		for (i=0;i<nvar;i++){
			...
			//yscal[i]=100.001;//fabs(y[i])+fabs(dydx[i]*h)+TINY;
			//cout << " i=" << i << " yscal[i]=" << yscal[i] << "   y[i]=" << y[i] << " dydx[i]=" << dydx[i] << " h*dydx[i]=" << h*dydx[i] << "  h=" << h << endl;//getchar();
		}
